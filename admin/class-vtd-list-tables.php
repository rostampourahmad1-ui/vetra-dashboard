...
class VTD_List_Tables {
	...
	public static function tickets() {
		...
		$items = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$table} {$where} ORDER BY ticket_id DESC LIMIT %d OFFSET %d", $per, ( $paged - 1 ) * $per ) );
	...
	public static function departments() {
		...
		$items = $wpdb->get_results( "SELECT * FROM {$table} ORDER BY department_id DESC" );
	...
	public static function notifications() {
		...
		$items = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$table} ORDER BY notification_id DESC LIMIT %d OFFSET %d", $per, ( $paged - 1 ) * $per ) );
	...
	public static function polls() {
		...
		$items = $wpdb->get_results( "SELECT * FROM {$table} ORDER BY poll_id DESC" );
	...
	public static function attachments() {
		...
		$items = $wpdb->get_results( "SELECT * FROM {$table} ORDER BY attachment_id DESC" );
	...
	public static function cards() {
		...
		$items  = $wpdb->get_results( "SELECT * FROM {$table} {$where} ORDER BY card_id DESC" );
	...
	public static function wallet() {
		...
		$wallets = $wpdb->get_results( 'SELECT w.*, u.display_name FROM ' . VTD_DB::wallets() . ' w LEFT JOIN ' . $wpdb->users . ' u ON u.ID = w.user_id ORDER BY w.balance DESC LIMIT 100' );
	...
	public static function sms_log() {
		...
		$total = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$table}" );
		$items = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$table} ORDER BY id DESC LIMIT %d OFFSET %d", $per, ( $paged - 1 ) * $per ) );
